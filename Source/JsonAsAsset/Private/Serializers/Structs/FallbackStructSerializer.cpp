...
void FFallbackStructSerializer::Deserialize(
    UScriptStruct *Struct, void *StructValue,
  ...
  for (FProperty *Property = Struct->PropertyLink; Property;
    ...
    if (PropertySerializer->ShouldDeserializeProperty(Property)) {
      ...
      const bool bHasHandledProperty = PassthroughPropertyHandler(
          Property, PropertyName, PropertyValue, JsonValue, PropertySerializer);

      if (!bHasHandledProperty && JsonValue->HasField(ValueName)) {
        const TSharedPtr<FJsonValue> *ValueObjectPtr =
            JsonValue->Values.Find(ValueName);
        if (ValueObjectPtr == nullptr || !ValueObjectPtr->IsValid()) {
          UE_LOG(LogJsonAsAsset, Warning,
                 TEXT("Skipping missing struct field '%s' while deserializing "
                      "'%s'."),
                 *ValueName, *Struct->GetName());
          continue;
        }
