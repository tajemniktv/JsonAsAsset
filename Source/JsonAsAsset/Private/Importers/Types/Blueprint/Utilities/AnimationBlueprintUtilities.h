...
inline FStructProperty *GetNodeStructProperty(const UAnimGraphNode_Base *Node) {
  ...
  for (TFieldIterator<FStructProperty> It(Node->GetClass()); It; ++It) {
    if (FStructProperty *StructProp = *It;
        StructProp->Struct &&
        StructProp->Struct->IsChildOf(FAnimNode_Base::StaticStruct())) {
      return StructProp;
    }
  }
...
inline void ReplaceLinkID(const TSharedPtr<FJsonValue> &Data,
                          const TArray<FString> &NodesKeys,
...
inline void
FilterAnimGraphNodeProperties(const TSharedPtr<FJsonObject> &JsonObject) {
...
inline TArray<TPair<FString, FString>>
FindLinkIDs(const TSharedPtr<FJsonValue> &Data,
            const FString &ParentProperty = TEXT("")) {
  ...
  if (Data->Type == EJson::Object) {
    ...
    for (auto &Pair : Object->Values) {
      ...
      TArray<TPair<FString, FString>> SubResults =
          FindLinkIDs(Pair.Value, Pair.Key);
    ...
    for (const TSharedPtr<FJsonValue> &Item : Arr) {
      TArray<TPair<FString, FString>> SubResults =
          FindLinkIDs(Item, ParentProperty);
...
inline void HarvestAndTagConnectedStateMachineNodes(
    const FString &StartKey, const FString &StateName,
    const FString &MachineName, TMap<FString, TSharedPtr<FJsonValue>> &Nodes) {
  if (!Nodes.Contains(StartKey))
    return;

  const TSharedPtr<FJsonValue> *NodeValuePtr = Nodes.Find(StartKey);
  if (NodeValuePtr == nullptr || !NodeValuePtr->IsValid()) {
    UE_LOG(LogJsonAsAsset, Warning,
           TEXT("State machine import: missing node '%s', skipping tag "
                "propagation."),
           *StartKey);
    return;
  }
  ...
  for (const TPair<FString, FString> &LinkPair : Links) {
    ...
    HarvestAndTagConnectedStateMachineNodes(NextKey, StateName, MachineName,
                                            Nodes);
  }
}

inline void
HandlePropertyBinding(FUObjectExport *NodeExport,
                      const TArray<TSharedPtr<FJsonValue>> &JsonObjects,
                      UAnimGraphNode_Base *Node, IImporter *Importer,
  ...
  if (NodeProperties->HasField(TEXT("EvaluateGraphExposedInputs"))) {
    const TSharedPtr<FJsonObject> EvaluateGraphExposedInputs =
        NodeProperties->GetObjectField(TEXT("EvaluateGraphExposedInputs"));
    if (!EvaluateGraphExposedInputs.IsValid() ||
        !EvaluateGraphExposedInputs->HasField(TEXT("BoundFunction"))) {
      return;
    }

    bool bBoundFunction = EvaluateGraphExposedInputs->GetStringField(
                              TEXT("BoundFunction")) != "None";

    if (EvaluateGraphExposedInputs->HasField(TEXT("CopyRecords")) ||
        bBoundFunction) {
      const TArray<TSharedPtr<FJsonValue>> CopyRecords =
          EvaluateGraphExposedInputs->HasField(TEXT("CopyRecords"))
      ...
      if (CopyRecords.Num() > 0) {
        for (const auto &CopyRecordAsValue : CopyRecords) {
          const TSharedPtr<FJsonObject> &CopyRecordAsObject =
              CopyRecordAsValue->AsObject();
          if (!CopyRecordAsObject.IsValid())
            continue;

          if (!CopyRecordAsObject->HasField(TEXT("DestProperty")))
            continue;
          if (CopyRecordAsObject->HasField(TEXT("BoundFunction")) &&
              CopyRecordAsObject->GetStringField(TEXT("BoundFunction")) !=
                  TEXT("None")) {
            bBoundFunction = true;
          ...
          FString SourcePropertyName =
              CopyRecordAsObject->GetStringField(TEXT("SourcePropertyName"));
           ...
           * Alpha
           */
          const TSharedPtr<FJsonObject> DestProperty =
              CopyRecordAsObject->GetObjectField(TEXT("DestProperty"));
          ...
          FString PinCategory =
              DestProperty->GetStringField(TEXT("ObjectName"));
          {
          ...
          /* Cannot be compiled */
          if (PinName.Equals(TEXT("BlendTime")) ||
              PinName.Equals(TEXT("BlendWeights")))
            continue;
          ...
          TSharedPtr<FJsonObject> SourcePropertyObject =
              GetExportMatchingWith(SourcePropertyName, "Name", JsonObjects);
          if (PinCategory == "struct" && SourcePropertyObject.IsValid() &&
              SourcePropertyObject->HasField(TEXT("Struct"))) {
            TSharedPtr<FJsonObject> StructObject =
                SourcePropertyObject->GetObjectField(TEXT("Struct"));
            ...
            FProperty *Prop =
                AnimClass->FindPropertyByName(*SourcePropertyName);

            if (FStructProperty *StructProp =
          ...
          if (CopyRecordAsObject->HasField(TEXT("SourceSubPropertyName")) &&
              CopyRecordAsObject->GetStringField(
                  TEXT("SourceSubPropertyName")) != "None") {
            FString SourceSubPropertyName = CopyRecordAsObject->GetStringField(
                TEXT("SourceSubPropertyName"));
            PropertyBinding.PathAsText = FText::FromString(
                SourcePropertyName + "." + SourceSubPropertyName);

            PropertyBinding.PropertyPath.Append({SourceSubPropertyName});

            if (CopyRecordAsObject->HasField(
                    TEXT("CachedSourceStructSubProperty"))) {
              TSharedPtr<FJsonObject> StructObject =
                  CopyRecordAsObject->GetObjectField(
                      TEXT("CachedSourceStructSubProperty"));
              ...
              if (StructProperty) {
                PropertyBinding.PinType.PinSubCategoryObject =
          ...
          if (PinName == "ActiveEnumValue" && Node != nullptr) {
            if (UAnimGraphNode_BlendListByEnum *BlendListByEnum =
                    Cast<UAnimGraphNode_BlendListByEnum>(Node)) {
              FProperty *Prop =
                  AnimClass->FindPropertyByName(*SourcePropertyName);
              ...
              } else if (FByteProperty *ByteProp =
                             CastField<FByteProperty>(Prop)) {
                EnumRef = ByteProp->Enum;
              }

              if (EnumRef) {
                FProperty *BoundEnumProp =
                    BlendListByEnum->GetClass()->FindPropertyByName(
                        TEXT("BoundEnum"));

                if (BoundEnumProp) {
                  if (FObjectProperty *ObjectProp =
                          CastField<FObjectProperty>(BoundEnumProp)) {
                    ObjectProp->SetObjectPropertyValue_InContainer(
                        BlendListByEnum, EnumRef);
                  }
                }
              }
            }
