...
bool IsVerboseBlueprintImport() {
  return GetBlueprintImportSettings().LogDetail ==
         EJBlueprintLogDetail::Verbose;
}
...
bool UseBlueprintCompatibilityFallback() {
  ...
  if (Settings && Settings->CompatibilityFallback.BlueprintGeneratedClass) {
    return true;
  }
...
bool IsBlueprintSafeToReuse(const UBlueprint *Blueprint) {
  if (!IsBlueprintObjectValid(Blueprint)) {
    return false;
  }

  if (HasPendingLoadFlags(Blueprint) ||
      HasPendingLoadFlags(Blueprint->GeneratedClass) ||
      HasPendingLoadFlags(Blueprint->SkeletonGeneratedClass)) {
    return false;
  }

  auto HasPendingGraphFlags = [](const TArray<UEdGraph *> &Graphs) {
    for (const UEdGraph *Graph : Graphs) {
      if (HasPendingLoadFlags(Graph)) {
        return true;
      }
    }
    return false;
  ...
  if (HasPendingGraphFlags(Blueprint->UbergraphPages) ||
      HasPendingGraphFlags(Blueprint->FunctionGraphs) ||
      HasPendingGraphFlags(Blueprint->DelegateSignatureGraphs) ||
      HasPendingGraphFlags(Blueprint->MacroGraphs)) {
    return false;
  }
...
bool IsLikelyFunctionExport(const FUObjectExport *Export) {
  if (!Export || !Export->IsJsonValid()) {
    return false;
  }

  const FString TypeName = Export->GetType().ToString();
  if (TypeName.Contains(TEXT("Function"))) {
    return true;
  }

  return Export->Has(TEXT("FunctionFlags")) || Export->Has(TEXT("ChildProperties"));
}
...
void GetAllSCSNodes(UBlueprint *InBlueprint, TArray<USCS_Node *> &OutNodes) {
  ...
  for (UBlueprint *ParentBlueprint = Cast<UBlueprint>(
           InBlueprint->ParentClass ? InBlueprint->ParentClass->ClassGeneratedBy
                                    : nullptr);
       ParentBlueprint; ParentBlueprint = Cast<UBlueprint>(
                            ParentBlueprint->ParentClass
                                ? ParentBlueprint->ParentClass->ClassGeneratedBy
                                : nullptr)) {
...
USCS_Node *FindSCSNodeByVariableName(const TArray<USCS_Node *> &Nodes,
...
UBlueprint *FindExistingBlueprint(UPackage *Package, const FString &AssetName) {
  ...
  if (UBlueprint *Existing = FindObject<UBlueprint>(Package, *AssetName)) {
    if (IsBlueprintSafeToReuse(Existing)) {
      return Existing;
    }

    UE_LOG(LogJsonAsAsset, Warning,
           TEXT("FindExistingBlueprint: '%s' is present but not fully "
                "loaded/safe to reuse; forcing recreate path."),
           *Existing->GetPathName());
    return nullptr;
  }

  /* Do not load the existing object here: forcing a load at this stage can
   * leave graph objects in a pending state. */
  return nullptr;
}
...
void MoveBlueprintToTransient(UBlueprint *Blueprint, const FString &Reason) {
  ...
  UE_LOG(LogJsonAsAsset, Warning,
         TEXT("Moving existing Blueprint '%s' to transient package: %s"),
...
UEdGraph *FindFunctionGraphByName(UBlueprint *Blueprint,
...
FName MakeSafeGraphName(UBlueprint *Blueprint, const FString &DesiredName,
                        const TCHAR *Suffix) {
  if (!Blueprint) {
  ...
  if (ExistingObject && !ExistingObject->IsA<UEdGraph>()) {
    ...
    const FName UniqueName = MakeUniqueObjectName(
        Blueprint, UEdGraph::StaticClass(), FName(*BaseName));
    UE_LOG(LogJsonAsAsset, Warning,
           TEXT("Blueprint graph name collision on '%s' with '%s'; using '%s'"),
           *DesiredName, *ExistingObject->GetClass()->GetName(),
           *UniqueName.ToString());
    return UniqueName;
  }
...
bool ResolvePinType(IBlueprintImporter *Importer,
  ...
  if (PropertyObject->HasField(TEXT("PropertyFlags"))) {
    ParseFlags(PropertyObject->GetStringField(TEXT("PropertyFlags")),
               PropertyFlags);
  }
  ...
  const auto LoadSubCategoryObject = [&](const TCHAR *FieldName,
                                         UObject *&OutObject) -> bool {
    OutObject = nullptr;
    ...
    TSharedPtr<FJsonObject> ObjectPath =
        PropertyObject->GetObjectField(FieldName);
  ...
  if (Type == TEXT("ArrayProperty") &&
      PropertyObject->HasField(TEXT("Inner"))) {
    FEdGraphPinType InnerPinType;
    if (!ResolvePinType(Importer, PropertyObject->GetObjectField(TEXT("Inner")),
                        InnerPinType)) {
      return false;
    }
  ...
  if (Type == TEXT("SetProperty") &&
      PropertyObject->HasField(TEXT("ElementProp"))) {
    FEdGraphPinType InnerPinType;
    if (!ResolvePinType(Importer,
                        PropertyObject->GetObjectField(TEXT("ElementProp")),
                        InnerPinType)) {
      return false;
    }
  ...
  if (Type == TEXT("MapProperty") &&
      PropertyObject->HasField(TEXT("KeyProp")) &&
      PropertyObject->HasField(TEXT("ValueProp"))) {
    FEdGraphPinType KeyPinType;
    FEdGraphPinType ValuePinType;
    if (!ResolvePinType(Importer,
                        PropertyObject->GetObjectField(TEXT("KeyProp")),
                        KeyPinType) ||
        !ResolvePinType(Importer,
                        PropertyObject->GetObjectField(TEXT("ValueProp")),
                        ValuePinType)) {
      return false;
    }
  ...
  if (Type == TEXT("Int8Property") || Type == TEXT("UInt16Property") ||
      Type == TEXT("UInt32Property") || Type == TEXT("UInt64Property") ||
      Type == TEXT("Int16Property")) {
    OutPinType.PinCategory = UEdGraphSchema_K2::PC_Int;
    return true;
  }
  ...
  if (Type == TEXT("WeakObjectProperty") || Type == TEXT("LazyObjectProperty")) {
    UObject *ObjectClass = nullptr;
    if (LoadSubCategoryObject(TEXT("PropertyClass"), ObjectClass)) {
      OutPinType.PinCategory = UEdGraphSchema_K2::PC_Object;
      OutPinType.PinSubCategoryObject = ObjectClass;
  ...
  if (Type == TEXT("MulticastDelegateProperty") ||
      Type == TEXT("MulticastInlineDelegateProperty") ||
      Type == TEXT("MulticastSparseDelegateProperty")) {
...
EPropertyFlags ResolvePropertyFlags(const TArray<FString> &Flags) {
  ...
  for (const FString &FlagString : Flags) {
    if (const EPropertyFlags *Resolved =
            PropertyFlagsByString.Find(FlagString)) {
      Result |= *Resolved;
    }
  }
...
EFunctionFlags ResolveFunctionFlags(const TArray<FString> &Flags) {
  ...
  for (const FString &FlagString : Flags) {
    if (const EFunctionFlags *Resolved =
            FunctionFlagsByString.Find(FlagString)) {
      Result |= *Resolved;
    }
  }
...
void CreateBlueprintMemberVariables(IBlueprintImporter *Importer,
                                    UBlueprint *Blueprint,
                                    const TSharedPtr<FJsonObject> &AssetData) {
  if (!Importer || !Blueprint || !AssetData.IsValid() ||
      !AssetData->HasField(TEXT("ChildProperties"))) {
    return;
  }

  /* Widget blueprints maintain widget-generated variables and can crash when
   * forced through generic member-variable creation. */
  if (Importer->GetAssetType().Contains(
          TEXT("WidgetBlueprintGeneratedClass"))) {
    if (IsVerboseBlueprintImport()) {
      UE_LOG(LogJsonAsAsset, Warning,
             TEXT("Skipping generic member variable reconstruction for Widget "
                  "Blueprint '%s'"),
             *Importer->GetAssetName());
    }
    return;
  }

  for (const TSharedPtr<FJsonValue> &PropertyValue :
       AssetData->GetArrayField(TEXT("ChildProperties"))) {
    if (!PropertyValue.IsValid() || PropertyValue->Type != EJson::Object ||
        !PropertyValue->AsObject().IsValid()) {
      continue;
    }

    const TSharedPtr<FJsonObject> PropertyObject = PropertyValue->AsObject();
    if (!PropertyObject.IsValid() || !PropertyObject->HasField(TEXT("Name")) ||
        !PropertyObject->HasField(TEXT("PropertyFlags"))) {
      continue;
    }

    const FString VariableNameString =
        PropertyObject->GetStringField(TEXT("Name"));
    ...
    /* Only member variables here; function params are handled while
    ...
    if (!ResolvePinType(Importer, PropertyObject, PinType)) {
      if (IsVerboseBlueprintImport()) {
        UE_LOG(LogJsonAsAsset, Warning,
               TEXT("Skipped unresolved Blueprint variable '%s'"),
               *VariableNameString);
      }
      continue;
    }
    ...
    for (const FBPVariableDescription &ExistingVariable :
    ...
    if (!bVariableExists) {
      if (!FBlueprintEditorUtils::AddMemberVariable(Blueprint, VariableName,
                                                    PinType)) {
        continue;
      }
    }

    uint64 *BlueprintVariableFlags =
        FBlueprintEditorUtils::GetBlueprintVariablePropertyFlags(Blueprint,
                                                                 VariableName);
    if (BlueprintVariableFlags) {
      *BlueprintVariableFlags |=
          static_cast<uint64>(ResolvePropertyFlags(PropertyFlags));
    }

    FBlueprintEditorUtils::SetBlueprintOnlyEditableFlag(
        Blueprint, VariableName,
        HasFlag(PropertyFlags, TEXT("DisableEditOnInstance")));
    FBlueprintEditorUtils::SetBlueprintPropertyReadOnlyFlag(
        Blueprint, VariableName,
        HasFlag(PropertyFlags, TEXT("BlueprintReadOnly")));
    FBlueprintEditorUtils::SetInterpFlag(
        Blueprint, VariableName, HasFlag(PropertyFlags, TEXT("Interp")));
    FBlueprintEditorUtils::SetVariableTransientFlag(
        Blueprint, VariableName, HasFlag(PropertyFlags, TEXT("Transient")));
    FBlueprintEditorUtils::SetVariableSaveGameFlag(
        Blueprint, VariableName, HasFlag(PropertyFlags, TEXT("SaveGame")));
    FBlueprintEditorUtils::SetVariableAdvancedDisplayFlag(
        Blueprint, VariableName,
        HasFlag(PropertyFlags, TEXT("AdvancedDisplay")));
    FBlueprintEditorUtils::SetVariableDeprecatedFlag(
        Blueprint, VariableName, HasFlag(PropertyFlags, TEXT("Deprecated")));

    if (HasFlag(PropertyFlags, TEXT("ExposeOnSpawn"))) {
      FBlueprintEditorUtils::SetBlueprintVariableMetaData(
          Blueprint, VariableName, nullptr,
          FBlueprintMetadata::MD_ExposeOnSpawn, TEXT("True"));
    }
  }
}

void SetupDynamicBindings(IBlueprintImporter *Importer, UBlueprint *Blueprint,
                          const TSharedPtr<FJsonObject> &AssetData) {
  if (!Importer || !Blueprint || !AssetData.IsValid() ||
      !AssetData->HasTypedField<EJson::Array>(TEXT("DynamicBindingObjects"))) {
    return;
  }
  ...
  for (const TSharedPtr<FJsonValue> &BindingPathValue :
       AssetData->GetArrayField(TEXT("DynamicBindingObjects"))) {
    if (!BindingPathValue.IsValid() ||
        BindingPathValue->Type != EJson::Object ||
        !BindingPathValue->AsObject().IsValid()) {
      continue;
    }

    const TSharedPtr<FJsonObject> BindingPathObject =
        BindingPathValue->AsObject();
    if (!BindingPathObject.IsValid()) {
      continue;
    }

    FUObjectExport *BindingExport =
        Importer->AssetContainer->GetExportByObjectPath(BindingPathObject);
    if (!BindingExport || !BindingExport->IsJsonValid() ||
        !BindingExport->Has(TEXT("Properties"))) {
      continue;
    }
    ...
    for (const FString &BindingArrayFieldName : BindingArrayFieldNames) {
      ...
      for (const TSharedPtr<FJsonValue> &DelegateBindingValue :
           BindingProps->GetArrayField(*BindingArrayFieldName)) {
        if (!DelegateBindingValue.IsValid() ||
            DelegateBindingValue->Type != EJson::Object ||
            !DelegateBindingValue->AsObject().IsValid()) {
          continue;
        }

        const TSharedPtr<FJsonObject> DelegateBinding =
            DelegateBindingValue->AsObject();
        if (!DelegateBinding.IsValid()) {
          continue;
        }

        FString FunctionName;
        if (!DelegateBinding->TryGetStringField(TEXT("FunctionNameToBind"),
                                                FunctionName) ||
        ...
        if (BindingArrayFieldName != TEXT("ComponentDelegateBindings")) {
          if (IsVerboseBlueprintImport()) {
            UE_LOG(LogJsonAsAsset, Log,
                   TEXT("Captured dynamic binding '%s' -> '%s'"),
        ...
        DelegateBinding->TryGetStringField(TEXT("DelegatePropertyName"),
                                           DelegatePropertyName);

        FString ComponentName;
        DelegateBinding->TryGetStringField(TEXT("ComponentPropertyName"),
                                           ComponentName);
...
void ApplyPinDefaultFromPropertyObject(IBlueprintImporter *Importer,
                                       UEdGraphPin *Pin,
                                       const TSharedPtr<FJsonObject> &PropertyObject) {
  if (!Importer || !Pin || !PropertyObject.IsValid()) {
    return;
  }

  FString DefaultValue;
  if (PropertyObject->TryGetStringField(TEXT("DefaultValue"), DefaultValue) &&
      !DefaultValue.IsEmpty()) {
    Pin->DefaultValue = DefaultValue;
  } else if (PropertyObject->TryGetStringField(TEXT("AutogeneratedDefaultValue"),
                                               DefaultValue) &&
             !DefaultValue.IsEmpty()) {
    Pin->DefaultValue = DefaultValue;
  }

  if (PropertyObject->HasTypedField<EJson::Object>(TEXT("DefaultObject"))) {
    TObjectPtr<UObject> LoadedDefaultObject;
    TSharedPtr<FJsonObject> DefaultObjectPath =
        PropertyObject->GetObjectField(TEXT("DefaultObject"));
    Importer->LoadExport(&DefaultObjectPath, LoadedDefaultObject);
    if (LoadedDefaultObject) {
      Pin->DefaultObject = LoadedDefaultObject.Get();
    }
  }
}

struct FBlueprintFunctionImportCandidate {
...
void AddFunctionCandidate(FUObjectExport *FunctionExport,
                          TSet<FString> &SeenFunctions,
                          TArray<FBlueprintFunctionImportCandidate> &OutCandidates) {
  if (!IsLikelyFunctionExport(FunctionExport)) {
    return;
  }

  const FString FunctionName = FunctionExport->GetName().ToString();
  if (FunctionName.IsEmpty() || SeenFunctions.Contains(FunctionName)) {
    return;
  }

  FBlueprintFunctionImportCandidate Candidate;
  Candidate.Export = FunctionExport;
  Candidate.FunctionName = FunctionName;

  if (FunctionExport->Has(TEXT("FunctionFlags"))) {
    ParseFlags(FunctionExport->GetString(TEXT("FunctionFlags")),
               Candidate.FunctionFlags);
  }

  Candidate.bEvent = HasFlag(Candidate.FunctionFlags, TEXT("FUNC_Event"));
  Candidate.bDelegate =
      HasFlag(Candidate.FunctionFlags, TEXT("FUNC_Delegate"));

  OutCandidates.Add(Candidate);
  SeenFunctions.Add(FunctionName);
}

void CollectFunctionImportCandidates(
    IBlueprintImporter *Importer,
    TArray<FBlueprintFunctionImportCandidate> &OutCandidates) {
  if (!Importer || !Importer->AssetContainer) {
    return;
  }

  const TSharedPtr<FJsonObject> AssetData = Importer->GetAssetData();
  if (!AssetData.IsValid()) {
    return;
  }

  TSet<FString> SeenFunctions;
  const bool bHasFuncMap = AssetData->HasTypedField<EJson::Object>(TEXT("FuncMap"));

  if (bHasFuncMap) {
    const TSharedPtr<FJsonObject> FuncMap = AssetData->GetObjectField(TEXT("FuncMap"));
    if (FuncMap.IsValid()) {
      for (const auto &Pair : FuncMap->Values) {
        ...
        FUObjectExport *FunctionExport =
            Importer->AssetContainer->GetExportByObjectPath(Pair.Value->AsObject());
        AddFunctionCandidate(FunctionExport, SeenFunctions, OutCandidates);
      }
    }
  }

  const bool bUseChildrenSupplement =
      !bHasFuncMap || UseBlueprintCompatibilityFallback();
  if (!bUseChildrenSupplement ||
      !AssetData->HasTypedField<EJson::Array>(TEXT("Children"))) {
    return;
  }

  for (const TSharedPtr<FJsonValue> &ChildValue :
       AssetData->GetArrayField(TEXT("Children"))) {
    if (!ChildValue.IsValid() || ChildValue->Type != EJson::Object ||
        !ChildValue->AsObject().IsValid()) {
      continue;
    }

    FUObjectExport *FunctionExport =
        Importer->AssetContainer->GetExportByObjectPath(ChildValue->AsObject());
    AddFunctionCandidate(FunctionExport, SeenFunctions, OutCandidates);
  }
}

void AddFunctionPinsFromChildProperties(IBlueprintImporter *Importer,
                                        UBlueprint *Blueprint, UEdGraph *Graph,
                                        const FUObjectExport *FunctionExport) {
  if (!Importer || !Blueprint || !Graph || !FunctionExport ||
      !FunctionExport->Has(TEXT("ChildProperties"))) {
    return;
  }
  ...
  for (const FUObjectJsonValueExport &ChildValue :
       FunctionExport->GetArray(TEXT("ChildProperties"))) {
    ...
    TArray<FString> PropertyFlags;
    ParseFlags(ChildObject->GetStringField(TEXT("PropertyFlags")),
               PropertyFlags);
    if (!HasFlag(PropertyFlags, TEXT("Parm"))) {
      continue;
    }
    ...
    if (!ResolvePinType(Importer, ChildObject, PinType)) {
      if (IsVerboseBlueprintImport()) {
        UE_LOG(LogJsonAsAsset, Warning,
               TEXT("Skipped unresolved function pin '%s' in '%s'"),
               *ChildObject->GetStringField(TEXT("Name")),
               *FunctionExport->GetName().ToString());
      }
      continue;
    }
    ...
    if (HasFlag(PropertyFlags, TEXT("OutParm")) ||
        HasFlag(PropertyFlags, TEXT("ReturnParm"))) {
      if (ResultNode->FindPin(PinName)) {
        continue;
      }
      ResultNode->CreateUserDefinedPin(PinName, PinType, EGPD_Output);
      ApplyPinDefaultFromPropertyObject(Importer, ResultNode->FindPin(PinName),
                                        ChildObject);
    } else {
      if (EntryNode->FindPin(PinName)) {
        continue;
      }
      EntryNode->CreateUserDefinedPin(PinName, PinType, EGPD_Input);
      ApplyPinDefaultFromPropertyObject(Importer, EntryNode->FindPin(PinName),
                                        ChildObject);
    }
  }
}

void AddCustomEventPinsFromChildProperties(
    IBlueprintImporter *Importer, UK2Node_CustomEvent *EventNode,
    const FUObjectExport *FunctionExport) {
  if (!Importer || !EventNode || !FunctionExport ||
      !FunctionExport->Has(TEXT("ChildProperties"))) {
    return;
  }
  ...
  for (const FUObjectJsonValueExport &ChildValue :
    ...
    const TSharedPtr<FJsonObject> ChildObject = ChildValue.JsonObject;
    if (!ChildObject.IsValid() || !ChildObject->HasField(TEXT("Name")) ||
        !ChildObject->HasField(TEXT("PropertyFlags"))) {
      continue;
    }
    ...
    const FName PinName(*ChildObject->GetStringField(TEXT("Name")));
    if (!EventNode->FindPin(PinName)) {
      EventNode->CreateUserDefinedPin(PinName, PinType, EGPD_Output);
      ApplyPinDefaultFromPropertyObject(Importer, EventNode->FindPin(PinName),
                                        ChildObject);
    }
  }
}
...
bool IBlueprintImporter::Import() {
  ...
  if (!ClassDefaultObjectExport || !ClassDefaultObjectExport->IsJsonValid() ||
    ...
    LogBlueprintImportWarning(
        FString::Printf(TEXT("Missing ClassDefaultObject for Blueprint '%s'"),
                        *GetAssetName()),
        true);
  ...
  if (!GeneratedClass) {
    LogBlueprintImportWarning(
        FString::Printf(TEXT("Missing generated class for Blueprint '%s'"),
                        *GetAssetName()),
        true);
    return false;
  }
  ...
  /* Apply Blueprint object fields before graph hookup to keep inherited/base
   * data stable first. */
  GetObjectSerializer()->DeserializeObjectProperties(
      RemovePropertiesShared(
          GetAssetData(),
          {"FuncMap", "SimpleConstructionScript", "ClassDefaultObject",
           "Children", "ChildProperties", "SuperStruct", "Super",
           "UberGraphFunction", "UberGraphFramePointerProperty"}),
      Blueprint);

  GetObjectSerializer()->DeserializeObjectProperties(
      ClassDefaultObjectExport->GetProperties(),
  ...
  TArray<FBlueprintFunctionImportCandidate> FunctionCandidates;
  CollectFunctionImportCandidates(this, FunctionCandidates);

  if (FunctionCandidates.Num() > 0) {
    ClearExistingEventNodes(Blueprint);

    bool bNeedsUberGraph = false;
    for (const FBlueprintFunctionImportCandidate &Candidate :
         FunctionCandidates) {
      if (Candidate.bEvent && !Candidate.bDelegate) {
        bNeedsUberGraph = true;
        break;
      }
    }

    UEdGraph *UberGraph = bNeedsUberGraph ? GetOrCreateUberGraph(Blueprint)
                                          : nullptr;
    if (bNeedsUberGraph && !UberGraph) {
      LogBlueprintImportWarning(
          FString::Printf(
              TEXT("Function graph reconstruction skipped for '%s': EventGraph creation failed."),
              *GetAssetName()),
          true);
    ...
    for (const FBlueprintFunctionImportCandidate &Candidate :
         FunctionCandidates) {
      FUObjectExport *FunctionExport = Candidate.Export;
      if (!FunctionExport || !FunctionExport->IsJsonValid()) {
        continue;
      }

      const FString &FunctionName = Candidate.FunctionName;
      const TArray<FString> &FunctionFlags = Candidate.FunctionFlags;

      if (!Candidate.bEvent && !Candidate.bDelegate) {
        UEdGraph *ExistingGraph =
            FindFunctionGraphByName(Blueprint, FunctionName);
        if (!ExistingGraph) {
          const FName SafeGraphName =
              MakeSafeGraphName(Blueprint, FunctionName, TEXT("_FuncGraph"));
          ExistingGraph = FBlueprintEditorUtils::CreateNewGraph(
              Blueprint, SafeGraphName, UEdGraph::StaticClass(),
              UEdGraphSchema_K2::StaticClass());
          FBlueprintEditorUtils::AddFunctionGraph<UFunction>(
              Blueprint, ExistingGraph, true, nullptr);
        }
        ...
        if (UK2Node_FunctionEntry *EntryNode = Cast<UK2Node_FunctionEntry>(
                FBlueprintEditorUtils::GetEntryNode(ExistingGraph))) {
          EntryNode->ClearExtraFlags(FUNC_AllFlags);
          EntryNode->SetExtraFlags(ResolveFunctionFlags(FunctionFlags));
          EntryNode->Modify();
        }

        AddFunctionPinsFromChildProperties(this, Blueprint, ExistingGraph,
                                           FunctionExport);
        continue;
      }

      if (Candidate.bDelegate) {
        UEdGraph *DelegateGraph =
            FindFunctionGraphByName(Blueprint, FunctionName);
        if (!DelegateGraph) {
          const FName SafeGraphName = MakeSafeGraphName(Blueprint, FunctionName,
                                                        TEXT("_DelegateGraph"));
        ...
        Schema->CreateFunctionGraphTerminators(*DelegateGraph,
                                               static_cast<UClass *>(nullptr));

        if (UK2Node_FunctionEntry *EntryNode = Cast<UK2Node_FunctionEntry>(
                FBlueprintEditorUtils::GetEntryNode(DelegateGraph))) {
          EntryNode->ClearExtraFlags(FUNC_AllFlags);
          EntryNode->SetExtraFlags(ResolveFunctionFlags(FunctionFlags));
          EntryNode->Modify();
        }

        AddFunctionPinsFromChildProperties(this, Blueprint, DelegateGraph,
                                           FunctionExport);
        continue;
      }
      ...
      if (FunctionExport->Has(TEXT("SuperStruct")) &&
          FunctionExport->GetObject(TEXT("SuperStruct")).JsonObject.IsValid()) {
        const FString SuperObjectName =
            FunctionExport->GetObject(TEXT("SuperStruct"))
                .GetString(TEXT("ObjectName"))
                .Replace(TEXT("Function'"), TEXT(""))
                .Replace(TEXT("'"), TEXT(""));
        FString EventName;
        SuperObjectName.Split(TEXT(":"), nullptr, &EventName);
        if (!EventName.IsEmpty() && Blueprint->ParentClass) {
          ParentFunction =
              Blueprint->ParentClass->FindFunctionByName(*EventName);
        }
      }

      if (!ParentFunction && Blueprint->ParentClass) {
        ParentFunction = Blueprint->ParentClass->FindFunctionByName(*FunctionName);
      }
      ...
      if (ParentFunction) {
        ...
        EventNode->EventReference.SetFromField<UFunction>(ParentFunction,
                                                          false);
        ...
        UK2Node_CustomEvent *CustomEventNode =
            NewObject<UK2Node_CustomEvent>(UberGraph);
        ...
        AddCustomEventPinsFromChildProperties(this, CustomEventNode,
                                              FunctionExport);
    ...
    UE_LOG(LogJsonAsAsset, Log,
           TEXT("Blueprint '%s' has no function candidates; skipping function/event graph reconstruction."),
           *GetAssetName());
  }
  ...
  if (ShouldCompileImmediately()) {
    ...
    UE_LOG(LogJsonAsAsset, Log,
           TEXT("Skipping immediate compile for Blueprint '%s' due to "
...
void IBlueprintImporter::SetupConstructionScript() const {
  ...
  GeneratedClass->SimpleConstructionScript =
      NewObject<USimpleConstructionScript>(GeneratedClass);
  ...
  USimpleConstructionScript *SimpleConstructionScript =
      GeneratedClass->SimpleConstructionScript;
  FUObjectExport *SimpleConstructionScriptExport =
      AssetContainer->GetExportByObjectPath(
          GetAssetDataAsValue().GetObject(TEXT("SimpleConstructionScript")));
  if (!SimpleConstructionScriptExport ||
      !SimpleConstructionScriptExport->IsJsonValid() ||
      !SimpleConstructionScriptExport->GetProperties().IsValid()) {
    LogBlueprintImportWarning(
        FString::Printf(
            TEXT("SimpleConstructionScript export missing for '%s'"),
            *GetAssetName()),
        true);
    return;
  }
  ...
  GetObjectSerializer()->DeserializeObjectProperties(
      SimpleConstructionScriptExport->GetProperties(),
...
bool IBlueprintImporter::SetupInheritableComponentHandler() const {
  if (!Blueprint ||
      !GetAssetDataAsValue().Has(TEXT("InheritableComponentHandler"))) {
    return true;
  }

  FUObjectExport *InheritableComponentHandlerExport =
      AssetContainer->GetExportByObjectPath(
          GetAssetDataAsValue().GetObject(TEXT("InheritableComponentHandler")));
  if (!InheritableComponentHandlerExport ||
      !InheritableComponentHandlerExport->IsJsonValid() ||
      !InheritableComponentHandlerExport->HasProperty(TEXT("Records"))) {
    return !IsStrictBlueprintImport();
  }

  UInheritableComponentHandler *ComponentHandler =
  ...
  for (const TSharedPtr<FJsonValue> &RecordValue :
       InheritableComponentHandlerExport->GetProperties()->GetArrayField(
           TEXT("Records"))) {
    const TSharedPtr<FJsonObject> RecordObject = RecordValue->AsObject();
    if (!RecordObject.IsValid() ||
        !RecordObject->HasField(TEXT("ComponentKey")) ||
        !RecordObject->HasField(TEXT("ComponentTemplate"))) {
      continue;
    }

    const TSharedPtr<FJsonObject> ComponentKey =
        RecordObject->GetObjectField(TEXT("ComponentKey"));
    if (!ComponentKey.IsValid() ||
        !ComponentKey->HasField(TEXT("SCSVariableName"))) {
      continue;
    }

    const FName SCSVariableName(
        *ComponentKey->GetStringField(TEXT("SCSVariableName")));
    USCS_Node *SCSNode =
        FindSCSNodeByVariableName(AllSCSNodes, SCSVariableName);
    if (!SCSNode) {
      if (IsVerboseBlueprintImport()) {
        UE_LOG(LogJsonAsAsset, Warning,
               TEXT("Inheritable component override skipped; SCS node '%s' not "
                    "found"),
               *SCSVariableName.ToString());
      }
      continue;
    }

    const FComponentKey CreatedComponentKey(SCSNode);
    UActorComponent *CreatedTemplate =
        ComponentHandler->CreateOverridenComponentTemplate(CreatedComponentKey);
    if (!CreatedTemplate) {
      continue;
    }

    FUObjectExport *ComponentTemplateExport =
        AssetContainer->GetExportByObjectPath(
            RecordObject->GetObjectField(TEXT("ComponentTemplate")));
    if (!ComponentTemplateExport || !ComponentTemplateExport->IsJsonValid() ||
        !ComponentTemplateExport->GetProperties().IsValid()) {
      continue;
    }

    GetObjectSerializer()->DeserializeObjectProperties(
        ComponentTemplateExport->GetProperties(), CreatedTemplate);
  }
...
bool IBlueprintImporter::SetupImplementedInterfaces() const {
  ...
  for (const TSharedPtr<FJsonValue> &InterfaceValue :
       GetAssetData()->GetArrayField(TEXT("Interfaces"))) {
    if (!InterfaceValue.IsValid() || InterfaceValue->Type != EJson::Object ||
        !InterfaceValue->AsObject().IsValid()) {
      continue;
    }

    const TSharedPtr<FJsonObject> InterfaceObject = InterfaceValue->AsObject();
    if (!InterfaceObject.IsValid() ||
        !InterfaceObject->HasField(TEXT("Class"))) {
      continue;
    }

    const TSharedPtr<FJsonObject> InterfaceClassObject =
        InterfaceObject->GetObjectField(TEXT("Class"));
    UClass *InterfaceClass = LoadClass(InterfaceClassObject);
    if (!InterfaceClass) {
      bAnyFailed = true;
      UE_LOG(LogJsonAsAsset, Warning,
             TEXT("Failed to load interface class '%s'"),
             *InterfaceClassObject->GetStringField(TEXT("ObjectPath")));
      continue;
    }
...
void IBlueprintImporter::SetupTimelineComponents() const {
  ...
  for (const TSharedPtr<FJsonValue> &TimelinePathValue :
       GetAssetData()->GetArrayField(TEXT("Timelines"))) {
    if (!TimelinePathValue.IsValid() ||
        TimelinePathValue->Type != EJson::Object ||
        !TimelinePathValue->AsObject().IsValid()) {
      continue;
    }

    const TSharedPtr<FJsonObject> TimelinePathObject =
        TimelinePathValue->AsObject();
    if (!TimelinePathObject.IsValid()) {
      continue;
    }

    FUObjectExport *TimelineExport =
        AssetContainer->GetExportByObjectPath(TimelinePathObject);
    if (!TimelineExport || !TimelineExport->IsJsonValid() ||
        !TimelineExport->GetProperties().IsValid()) {
      continue;
    }

    const TSharedPtr<FJsonObject> TimelineProperties =
        TimelineExport->GetProperties();
    if (!TimelineProperties.IsValid() ||
        !TimelineProperties->HasField(TEXT("VariableName"))) {
      continue;
    }

    const FName TimelineVariableName(
        *TimelineProperties->GetStringField(TEXT("VariableName")));
    if (TimelineVariableName.IsNone()) {
      continue;
    }

    int32 TimelineIndex = FBlueprintEditorUtils::FindTimelineIndex(
        Blueprint, TimelineVariableName);
    UTimelineTemplate *TimelineTemplate = nullptr;
    if (TimelineIndex != INDEX_NONE &&
        Blueprint->Timelines.IsValidIndex(TimelineIndex)) {
      TimelineTemplate = Blueprint->Timelines[TimelineIndex].Get();
    } else {
      TimelineTemplate = FBlueprintEditorUtils::AddNewTimeline(
          Blueprint, TimelineVariableName);
    }
    if (!TimelineTemplate) {
      continue;
    ...
    if (TimelineProperties->HasField(TEXT("TimelineGuid"))) {
      TimelineTemplate->TimelineGuid =
          FGuid(TimelineProperties->GetStringField(TEXT("TimelineGuid")));
    }

    if (TimelineProperties->HasTypedField<EJson::Number>(
            TEXT("TimelineLength"))) {
      TimelineTemplate->TimelineLength =
          TimelineProperties->GetNumberField(TEXT("TimelineLength"));
    }
    if (TimelineProperties->HasTypedField<EJson::Boolean>(TEXT("bAutoPlay"))) {
      TimelineTemplate->bAutoPlay =
          TimelineProperties->GetBoolField(TEXT("bAutoPlay"));
    }
    ...
    if (TimelineProperties->HasTypedField<EJson::Boolean>(
            TEXT("bIgnoreTimeDilation"))) {
      TimelineTemplate->bIgnoreTimeDilation =
          TimelineProperties->GetBoolField(TEXT("bIgnoreTimeDilation"));
    }
    if (TimelineProperties->HasTypedField<EJson::Boolean>(
            TEXT("bReplicated"))) {
      TimelineTemplate->bReplicated =
          TimelineProperties->GetBoolField(TEXT("bReplicated"));
    }
    ...
    auto AddTrackCurves = [&](const FString &FieldName,
      ...
      for (const TSharedPtr<FJsonValue> &TrackValue :
        ...
        if (TrackType == FTTTrackBase::TT_FloatInterp) {
          FTTFloatTrack Track;
          Track.CurveFloat =
              NewObject<UCurveFloat>(Blueprint, UCurveFloat::StaticClass(),
          ...
          Track.CurveVector =
              NewObject<UCurveVector>(Blueprint, UCurveVector::StaticClass(),
                                      NAME_None, RF_Transactional);
          Track.SetTrackName(TrackName, TimelineTemplate);
          TimelineTemplate->VectorTracks.Add(Track);
        } else if (TrackType == FTTTrackBase::TT_LinearColorInterp) {
          FTTLinearColorTrack Track;
          Track.CurveLinearColor = NewObject<UCurveLinearColor>(
              Blueprint, UCurveLinearColor::StaticClass(), NAME_None,
              RF_Transactional);
          Track.SetTrackName(TrackName, TimelineTemplate);
    ...
    AddTrackCurves(TEXT("LinearColorTracks"),
                   FTTTrackBase::TT_LinearColorInterp);
    ...
    if (EventGraph) {
      ...
      for (UEdGraphNode *ExistingNode : EventGraph->Nodes) {
        if (UK2Node_Timeline *ExistingTimelineNode =
                Cast<UK2Node_Timeline>(ExistingNode)) {
      ...
      if (!bTimelineNodeExists) {
        ...
        TimelineNode->bIgnoreTimeDilation =
            TimelineTemplate->bIgnoreTimeDilation;
...
UObject *IBlueprintImporter::CreateAsset(UObject *CreatedAsset) {
  ...
  if (!ParentClass) {
    AppendNotification(
        ...
        FText::FromString("The parent Blueprint's class could not be found. "
  ...
  FModuleManager::LoadModuleChecked<IKismetCompilerInterface>("KismetCompiler")
      .GetBlueprintTypesForClass(ParentClass, BlueprintClass, GeneratedClass);
  ...
  if (Existing && Policy == EJBlueprintReimportPolicy::AlwaysRecreate) {
    MoveBlueprintToTransient(Existing,
                             TEXT("Reimport policy is AlwaysRecreate."));
    Existing = nullptr;
  }

  if (Existing && Policy == EJBlueprintReimportPolicy::RecreateInvalid &&
      !IsBlueprintSafeToReuse(Existing)) {
    MoveBlueprintToTransient(
        Existing, TEXT("Existing Blueprint is partially constructed."));
    Existing = nullptr;
  }

  if (Existing && Policy == EJBlueprintReimportPolicy::ReuseValid &&
      !IsBlueprintSafeToReuse(Existing)) {
    if (IsStrictBlueprintImport()) {
      UE_LOG(LogJsonAsAsset, Error,
             TEXT("ReuseValid policy prevented recreate of invalid Blueprint "
                  "'%s'."),
             *GetAssetName());
      return nullptr;
    }
    ...
    MoveBlueprintToTransient(
        Existing,
        TEXT("ReuseValid fallback: invalid Blueprint moved out of package."));
    Existing = nullptr;
  }
   ...
   */
  if (UBlueprint *Conflicting =
          FindObject<UBlueprint>(GetPackage(), *GetAssetName())) {
    if (HasPendingLoadFlags(Conflicting)) {
      UE_LOG(LogJsonAsAsset, Error,
             TEXT("CreateBlueprint blocked for '%s': conflicting object '%s' "
                  "is still pending load."),
             *GetAssetName(), *Conflicting->GetPathName());
      AppendNotification(
          FText::FromString("Blueprint Import Deferred"),
          FText::FromString(
              "Existing Blueprint is not fully loaded yet. Reopen project or "
              "delete the partial asset and retry."),
          4.0f, SNotificationItem::CS_Fail, true, 450.0f);
      return nullptr;
    }

    MoveBlueprintToTransient(
        Conflicting, TEXT("Conflicting object exists before CreateBlueprint."));
  }

  UBlueprint *NewBlueprint = FKismetEditorUtilities::CreateBlueprint(
      ParentClass, GetPackage(), FName(*GetAssetName()),
      GetBlueprintType(ParentClass), BlueprintClass, GeneratedClass);

  if (!NewBlueprint) {
    UE_LOG(LogJsonAsAsset, Error, TEXT("CreateBlueprint failed for '%s'"),
           *GetAssetName());
    return nullptr;
  }
