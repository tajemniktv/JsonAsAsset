...
bool FTextureCreatorUtilities::CreateTexture(UTexture*& OutTexture, TArray<uint8>& Data, const TSharedPtr<FJsonObject>& Properties) {
	...
	if (Properties->TryGetArrayField(TEXT("Mips"), TextureMipsPtr)) {
#else
...
#if UE4_18_BELOW
	if (Properties->TryGetStringField(TEXT("AddressX"), AddressX)) {
		UEnum* Enum = FindObject<UEnum>(ANY_PACKAGE, TEXT("TextureAddress"));
		if (Enum) RenderTarget2D->AddressX = static_cast<TextureAddress>(Enum->GetValueByNameString(AddressX));
	}
	if (Properties->TryGetStringField(TEXT("AddressY"), AddressY)) {
		UEnum* Enum = FindObject<UEnum>(ANY_PACKAGE, TEXT("TextureAddress"));
		if (Enum) RenderTarget2D->AddressY = static_cast<TextureAddress>(Enum->GetValueByNameString(AddressY));
	}
	if (Properties->TryGetStringField(TEXT("RenderTargetFormat"), RenderTargetFormat)) {
		UEnum* Enum = FindObject<UEnum>(ANY_PACKAGE, TEXT("ETextureRenderTargetFormat"));
		if (Enum) RenderTarget2D->RenderTargetFormat = static_cast<ETextureRenderTargetFormat>(Enum->GetValueByNameString(RenderTargetFormat));
	}
#else
...
bool FTextureCreatorUtilities::DeserializeTexture2D(UTexture2D* InTexture2D, const TSharedPtr<FJsonObject>& Properties) const {
	...
	if (Properties->TryGetStringField(TEXT("AddressX"), AddressX)) {
		UEnum* Enum = FindObject<UEnum>(ANY_PACKAGE, TEXT("TextureAddress"));
		if (Enum) InTexture2D->AddressX = static_cast<TextureAddress>(Enum->GetValueByNameString(AddressX));
	}
	if (Properties->TryGetStringField(TEXT("AddressY"), AddressY)) {
		UEnum* Enum = FindObject<UEnum>(ANY_PACKAGE, TEXT("TextureAddress"));
		if (Enum) InTexture2D->AddressY = static_cast<TextureAddress>(Enum->GetValueByNameString(AddressY));
	}
#else
	if (Properties->TryGetStringField(TEXT("AddressX"), AddressX)) InTexture2D->AddressX = static_cast<TextureAddress>(StaticEnum<TextureAddress>()->GetValueByNameString(AddressX));
	if (Properties->TryGetStringField(TEXT("AddressY"), AddressY)) InTexture2D->AddressY = static_cast<TextureAddress>(StaticEnum<TextureAddress>()->GetValueByNameString(AddressY));
